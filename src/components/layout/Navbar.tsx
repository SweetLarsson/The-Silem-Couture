              ...
              <AnimatePresence>
                {isDesktopLangOpen && (
                  <motion.div 
                    ...
                    className="absolute top-full right-0 mt-2 w-40 bg-white drop-shadow-2xl rounded-[1.5rem] border border-gray-100 py-2 z-50 flex flex-col overflow-hidden"
                  >
                    {languages.map((lang) => (
                      <button
                        key={lang.code}
                ...
                >
                  {navLinks.map((link) => (
                    <Link 
                      key={link.name} 
                      to={link.path}
                      className="text-black text-sm font-bold uppercase tracking-wider hover:text-peach-500 transition-colors"
                      onClick={() => {
                  ...
                  <div className="pt-5 border-t border-gray-100">
                    <div className="relative w-full">
                      <button 
                        className="flex items-center justify-between px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl w-full text-black"
                      ...
                      <AnimatePresence>
                        {isMobileLangOpen && (
                          <motion.div 
                            ...
                            className="overflow-hidden mt-2 bg-gray-50 border border-gray-200 rounded-2xl flex flex-col w-full"
                          >
                            {languages.map((lang) => (
                              <button
                                key={lang.code}
